...
class bot:
    ...
    def report(self, option=None):
        ...
        if option:
            ...
            message += f"미국 달러 환율 : {self.indexs[self.USA][self.NUM]:,.2f}원 (전일 대비 {self.indexs[self.USA][self.CHANGE]}, 금일 최고가 : {self.indexs[self.USA]["high"]}원, 금일 최저가 : {self.indexs[self.USA]["low"]}원)" if self.indexs[self.USA]["show"] else ""
            message += f"일본 엔화 환율 : {self.indexs[self.JAP][self.NUM]:,.2f}원 (전일 대비 {self.indexs[self.JAP][self.CHANGE]}, 금일 최고가 : {self.indexs[self.JAP]["high"]}원, 금일 최저가 : {self.indexs[self.JAP]["low"]}원)" if self.indexs[self.JAP]["show"] else ""
            message += f"유럽 유로 환율 : {self.indexs[self.EU][self.NUM]:,.2f}원 (전일 대비 {self.indexs[self.EU][self.CHANGE]}, 금일 최고가 : {self.indexs[self.EU]["high"]}원, 금일 최저가 : {self.indexs[self.EU]["low"]}원)" if self.indexs[self.EU]["show"] else ""
            message += f"중국 위안화 환율 : {self.indexs[self.CN][self.NUM]:,.2f}원 (전일 대비 {self.indexs[self.CN][self.CHANGE]}, 금일 최고가 : {self.indexs[self.CN]["high"]}원, 금일 최저가 : {self.indexs[self.CN]["low"]}원)" if self.indexs[self.CN]["show"] else ""
            message += f"국제 유가 : {self.indexs[self.WOIL][self.NUM]:,.2f}달러 (전일 대비 {self.indexs[self.WOIL][self.CHANGE]}, 금일 최고가 : {self.indexs[self.WOIL]["high"]}달러, 금일 최저가 : {self.indexs[self.WOIL]["low"]}달러)" if self.indexs[self.WOIL]["show"] else ""
            message += f"국내 유가 : {self.indexs[self.KOIL][self.NUM]:,.2f}원 (전일 대비 {self.indexs[self.KOIL][self.CHANGE]}, 금일 최고가 : {self.indexs[self.KOIL]["high"]}원, 금일 최저가 : {self.indexs[self.KOIL]["low"]}원)" if self.indexs[self.KOIL]["show"] else ""
            message += f"국제 금 시세 : {self.indexs[self.WGOLD][self.NUM]:,.2f}달러 (전일 대비 {self.indexs[self.WGOLD][self.CHANGE]}, 금일 최고가 : {self.indexs[self.WGOLD]["high"]}달러, 금일 최저가 : {self.indexs[self.WGOLD]["low"]}달러)" if self.indexs[self.WGOLD]["show"] else ""
